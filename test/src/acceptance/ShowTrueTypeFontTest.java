...
public final class ShowTrueTypeFontTest {
	...
	public void showBitstream() throws IOException, DataFormatException {
		final File sourceDir = new File("test/data/ttf/reference/bitstream-vera");
	...
	public void showDejaVu() throws IOException, DataFormatException {
		...
		final File destDir = new File("test/results/ShowTrueTypeFontTest/ttf/dejavu");

		showFiles(sourceDir, sourceDir.list(filter), destDir);
	}

	private void showFiles(final File sourceDir, final String[] files, final File destDir)
	...
	private void showFont(final DefineFont2 font, final File file) throws IOException,
			DataFormatException {
