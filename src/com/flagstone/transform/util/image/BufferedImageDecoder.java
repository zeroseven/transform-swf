...
public final class BufferedImageDecoder implements ImageProvider, ImageDecoder {
	 ...
	 */
	public static BufferedImage resizeImage(final BufferedImage image, final int width,
			final int height) {
		...
		final double height_ratio = (double) image.getHeight() / (double) height;
	...
	private static byte[] unzip(final byte[] bytes, final int width, final int height)
			throws DataFormatException {
